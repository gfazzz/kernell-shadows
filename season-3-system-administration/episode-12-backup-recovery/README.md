...
**Персонажи:**
- **Max Sokolov** — вы, изучаете backup strategies
- **Liisa Kask** — backup engineer, ex-Skype (главный ментор)
- **Kristjan Tamm** — e-Government architect (поддержка)
- **Anna Kovaleva** — forensics expert (remote, координирует incident response)
...
Max спит в отеле в Старом городе Таллина. Резкий звонок телефона.

**Anna Kovaleva** (голос напряжённый):
*"Max, проснись. НЕМЕДЛЕННО. Атака. Krylov. Production server скомпрометирован. База данных удалена. Нужен restore. СЕЙЧАС."*

**Max** (дезориентирован, включает laptop):
...
Max звонит Liisa (03:52, ночь).

**Liisa** (сонный голос, потом резко просыпается):
*"Max? Что случилось?"*

**Max:**
*"Атака Krylov. База удалена. Anna сказала звонить тебе. У нас есть backup?"*
...
**Anna:**
*"Krylov оставил backdoor. Я нашла: `/var/lib/.krylov_backdoor.sh`. Запускался каждые 5 минут через cron, удалял данные постепенно. Forensics показывает: начало атаки — 3 дня назад. Они были внутри 72 часа."*
