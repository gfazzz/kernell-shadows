...
**Персонажи:**
- **Max Sokolov** — вы, растёте как sysadmin
- **Andrei Volkov** — ex-профессор Unix (ЛЭТИ), ментор
...
После успешного завершения Season 2 (VPN setup) Max возвращается в Россию. Но не в Новосибирск, а в Санкт-Петербург.

**Viktor** (видеозвонок, 03:27 Moscow time):
*"Макс, у нас проблема. Серьёзная. Один из серверов скомпрометирован."*

**Anna** (screen share — forensics report):
*"Backdoor. `/usr/sbin/sshd_backup` — fake sshd процесс. Krylov проник через учётную запись с sudo правами. Проверь `/etc/passwd` — там учётка `devops` с UID 0. ROOT ACCESS."*

**Max:** *"Как это возможно?!"*

**Alex:** *"Permissions. Кто-то дал sudo кому не следует. Классическая ошибка. Едь в СПб — там есть человек, который научит тебя правильному администрированию."*

---

### 02:00. Санкт-Петербург. ЛЭТИ.

Белые ночи. На улице светло, хотя 2 часа ночи. Max идёт по Васильевскому острову, канал Грибоедова за окнами старого здания ЛЭТИ. Встреча с **Andrei Volkov** в лаборатории Unix.

**Andrei** (седые волосы, очки, твидовый пиджак, чай в стеклянном стакане):
*"Максим. Viktor рассказал о проблеме. Permissions — это философия, а не просто команды. Principle of least privilege — слышал?"*
...
**Viktor** (звонок):
*"Макс, на сервере будут работать 5 человек: я, ты, Alex, Anna, Dmitry. Каждому нужен свой уровень доступа. Alex — только network команды. Anna — только чтение логов. Dmitry — управление сервисами. Настрой это правильно."*
...
- Alex может выполнять network команды через sudo (но НЕ всё)
- Anna может читать логи, но НЕ изменять
- Shared папка доступна всей команде
- Krylov больше не сможет эксплуатировать misconfigured permissions

**Итог:** Security audit report для Viktor.
...
**Andrei:**
*"Процессы. Systemd. Но это уже для Boris. Он фанатик systemd. Встретишься с ним завтра."*
