...
- **Alex Sokolov** — security lead, эмоциональный backstory
- **Anna Kovaleva** — forensics, анализ угроз
- **Dmitry Orlov** — DevOps, VPN infrastructure
- **Katarina Lindström** — криптография (Stockholm University)
...
- **Эмоциональный момент:** Alex открывается Max о прошлом:
  - Работал в ФСБ Управление "К" (киберпреступления)
  - Krylov (его начальник) фабриковал дела против невиновных
  - Alex отказался подписывать протокол → увольнение
  - Krylov: "Предатель. Ты пожалеешь."
  - Теперь Krylov охотится на Alex, Viktor, и всю команду
- Результат: Безопасная коммуникация через швейцарский сервер
- **Season 2 Complete:** Команда защищена, но Krylov ещё активен
