...
**Контекст:**
Виктор: *"У меня есть сервер в этом ЦОДе. Hostname: `shadow-server-02.ops.internal`. Узнай его IP адрес."*
...
**Контекст:**
Анна: *"Макс, нам нужно знать маршрут до сервера Viktor (shadow-server). Через какие узлы идёт трафик? Сколько роутеров между нами и им?"*
...
**Контекст:**
Дмитрий: *"Макс, нам нужно знать какие порты открыты на сервере Viktor (shadow-server). Какие сервисы работают? SSH? HTTP? База данных?"*
...
**Контекст:**
Алекс: *"Макс, теперь нужно просканировать сервер Viktor (shadow-server). Узнать какие порты открыты на НЁМ. Это то, как атакующий видит сервер. Используй nmap."*
...
**Контекст:**
Дмитрий: *"Макс, нужно проверить routing table. Через какой gateway идёт трафик? Это важно для настройки VPN (Episode 08)."*
...
**Контекст:**
Виктор: *"Макс, собери всё в отчёт. IP адреса, маршруты, открытые порты. Это наша карта сети. Она критична для защиты."*
...
> **Alex:**
> *"Макс, завтра летишь в Стокгольм. Erik Johansson встретит. Он эксперт по DNS и BGP. Баhnhof datacenter — бывший ядерный бункер. Там безопаснее."*
