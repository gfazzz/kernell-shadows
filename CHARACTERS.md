...
- Отец — физик в институте ядерной физики (умер когда Max было 15)
...
- Семейная лояльность (Alex просит о помощи)
...
**Связи:**
- Нанял Alex Sokolov (ex-FSB) для security
...
- Отправляет Max в новые локации
- Предупреждает об угрозах
- Финансирует операцию

**Тайна:**
Что это за операция? Почему Viktor нужна такая инфраструктура? Кто он на самом деле?
...
**Полное имя:** Александр "Alex" Соколов
**Возраст:** 33 года
**Национальность:** Русский
...
- Двоюродный брат Max (сын брата отца Max)
- Старше Max на 5 лет, всегда защищал его в детстве
- Образование: ФСБ Academy, компьютерная безопасность
- ФСБ Управление "К" (киберпреступность) — 2014-2023
- Звание: майор (до ухода)
- Ушёл из ФСБ в 2023 (конфликт с полковником Krylov)
...
- Защищает Max (семейная лояльность)
- Циничный (видел много дерьма в ФСБ)
- Не доверяет властям
- Готов на многое ради семьи

**Конфликт:**
Полковник Krylov (бывший начальник) охотится на него за "предательство" (уход из ФСБ).

Alex отказался фабриковать дела против невинных хакеров → конфликт с Krylov → увольнение → работа на Viktor.

**Личное:**
- Чувствует вину за то, что втянул Max в опасную операцию
- Но доверяет Max больше, чем кому-либо
- В S5 показывает уязвимость: *"Я покинул ФСБ, но методы остались. Иногда я вижу лица коллег. Которых я предал."*

**Функция в курсе:**
- Ментор Max по security (S5)
- Защита от Krylov
...
> *"Я знаю, как Krylov атакует. Потому что я сам так делал в ФСБ."*
...
- Работала с Alex (коллеги)
- Вышла одновременно с Alex (та же история с Krylov)
...
- Обучает Max forensics после инцидентов
...
- Знаком с Max по Новосибирскому Linux User Group
...
- Dmitry часто первый, кто объясняет новые концепции Max
...
**Создатель:** Viktor Petrov
**Специализация:** Linux SysAdmin, Networking, Security, Pentesting
**Роль:** AI-наставник Max, 24/7 техническая поддержка
...
**Функция:**
Обучает Max основам shell scripting, передаёт старую школу автоматизации.
...
**Функция:**
Помогает Max с анализом огромных логов (100MB+), обучает grep/awk/sed.
...
**Функция:**
Обучает Max advanced DNS protection, BGP routing, скандинавский подход к privacy.
...
**Функция:**
Обучает Max правильной настройке permissions, sudo, ACL.
...
**Функция:**
Обучает Max LVM, RAID, высоконадёжные системы.
...
**Появление:** Episode 12 (Backup & Recovery)

**Биография:**
- Работала в Skype (Skype основан в Таллине)
- После Microsoft acquisition перешла во фриланс
- Специализация: backup strategies, disaster recovery

**Характер:**
- Параноидная насчёт backup'ов
- "Если нет backup, данных не существует"
- Весёлая, но серьёзная о работе

**Функция:**
Обучает Max rsync, backup strategies, disaster recovery testing.

**Цитата:**
> *"Backup'ы как страховка. Надеешься, что не понадобятся, но рад, что есть. У меня 3 копии всего."*
...
**Функция:**
Обучает Max Git, знакомит с CCC philosophy, hacker ethics.
...
**Функция:**
Обучает Max Docker best practices, оптимизация образов.
...
**Функция:**
Обучает Max Ansible, Infrastructure as Code, идемпотентность.
...
- Консультирует Viktor по криптографии (с S2)
...
**Функция:**
Обучает Max threat modeling, SSL/TLS для банковского уровня, криптографию.
...
**Биография:**
- Финансирует серверы для Viktor
...
**Функция:**
Обучает Max threat modeling, STRIDE methodology.
...
**Функция:**
Обучает Max web security, защита критической научной инфраструктуры.
...
**Функция:**
Помогает Max с forensics после APT атаки, обучает incident response.
...
**Функция:**
Обучает Max embedded Linux, Raspberry Pi, GPIO, китайский подход к hardware.
...
**Функция:**
Обучает Max ArduPilot, MAVLink, автономные полёты.
...
**Функция:**
Обучает Max MQTT, IoT integration, security для IoT.
...
**Функция:**
Обучает Max Kubernetes, scaling, SRE practices.
...
**Функция:**
Обучает Max Prometheus, Grafana, alerting, SLO/SLI.
...
**Функция:**
Обучает Max SELinux, Fail2ban, Auditd, максимальный hardening.
...
- Звание: полковник
- Бывший начальник Alex и Anna Sokolov
- Не простил им "предательство" (уход из ФСБ в 2023)

**Характер:**
- Жестокий, беспринципный
- Фабриковал дела против хакеров (за это Alex с ним поссорился)
- Персональная вендетта против Alex и Anna
...
- Атаки на серверы (технические вызовы для Max)
- Финальный противостояние с Alex (S8)

**Финал:**
Физическая конфронтация с Alex в московском ЦОДе. Отступает (арестован? бежал? оставлено открытым для возможного продолжения).
...
- Social engineering
- Координированные атаки с Krylov (S8)
...
**По типам:**
- Core Team: 6 (Max, Viktor, Alex, Anna, Dmitry, LILITH)
- Local Experts: 18
- Antagonists: 3 (Krylov, "Новая Эра", The Architect)

**По локациям:**
- Новосибирск: 2 (Sergey, Olga)
- Москва: 3 (Alex, Anna, + Viktor HQ)
- Стокгольм: 2 (Erik, Katarina)
- СПб/Таллин: 3 (Andrei, Kristjan, Liisa)
...
### Функциональность:
- Каждый персонаж обучает Max чему-то конкретному
