...
1. **Читай теорию** в этом README (секция выше + Практика ниже)
2. **Открой `starter/README.md`** — там полный workflow
3. **Заполни TODO** в Docker конфигах (`starter/`)
4. **Build образы** через `docker build` и `docker-compose`
5. **Тестируй** контейнеры
6. **Запусти автотесты** `./tests/test.sh`

**LILITH:** *"Type B episode. Ты пишешь Dockerfile и docker-compose.yml напрямую, не bash wrapper. starter/ содержит конфиги с TODO комментариями. Заполни их, собери образы, запусти контейнеры. Так работают профессионалы."*
